...
{
	let result = mark::intersect({{-1, -1}, {1, 1}}, {{-1, 1}, {1, -1}});
	REQUIRE(result.has_value());
	REQUIRE(result->x == Approx(0));
	REQUIRE(result->y == Approx(0));
}
...
{
	let result = mark::intersect({{1, 1}, {1, -1}}, {{-1, 0}, {1, 0}});
	REQUIRE(result.has_value());
	REQUIRE(result->x == Approx(1));
	REQUIRE(result->y == Approx(0));
}
...
{
	let result = mark::intersect({{-1, 1}, {-1, -1}}, {{1, 1}, {0, 0}});
	REQUIRE(!result.has_value());
}
...
{
	let result = mark::intersect({{-5, 0}, {5, 0}}, {0, 0}, 1);
	REQUIRE(result.has_value());
	REQUIRE(result->x == Approx(-1));
	REQUIRE(result->y == Approx(0));
}
...
{
	let result = mark::intersect({{5, 3}, {7, 3}}, {5, 3}, 1);
	REQUIRE(result.has_value());
...
{
	let result = mark::intersect({{0, -5}, {0, 5}}, {0, 0}, 1);
	REQUIRE(result.has_value());
	REQUIRE(result->x == Approx(0));
...
{
	let result = mark::intersect({{0, 0}, {1, 1}}, {2, 2}, 2);
	REQUIRE(result.has_value());
...
{
	let result = mark::intersect({{8, 8}, {4.5, 4.5}}, {2, 2}, 2);
	REQUIRE(!result.has_value());
}
