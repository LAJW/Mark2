...
void mark::unit::heat_seeker::tick(mark::tick_context& context) {
	...
	if (enemy || !m_world.map().traversable(m_pos)) {
		...
		for (int i = 0; i < 80; i++) {
			auto direction = m_world.resource_manager().random(-180.f, 180.f);
			auto speed = m_world.resource_manager().random(5.f, 75.f);
	...
	for (int i = 0; i < 4; i++) {
		...
		float direction = m_world.resource_manager().random(-15.f, 15.f) + 180.f + m_rotation;
